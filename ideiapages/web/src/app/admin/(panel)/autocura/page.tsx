...
import { toAutocuraQueueDtos, toAutocuraStateDto } from "./serialise-props";

export const metadata = {
  title: "Autocura · IDeiaPages",
};

type AutomationStateRow = Database["public"]["Tables"]["automation_state"]["Row"];
...
export default async function AutocuraPage() {
  ...
  let initialState: ReturnType<typeof toAutocuraStateDto> = null;
  let initialQueue: ReturnType<typeof toAutocuraQueueDtos> = [];
  ...
  try {
    ...
    if (sRes.error) {
      ...
      initialState = toAutocuraStateDto((sRes.data as AutomationStateRow | null) ?? null);
    }

    if (qRes.error) {
      loadError = loadError ? `${loadError} · ${qRes.error.message}` : qRes.error.message;
    } else {
      initialQueue = toAutocuraQueueDtos((qRes.data as unknown[] | null) ?? []);
    }
  } catch (e) {
    loadError = e instanceof Error ? e.message : "Falha ao carregar dados de autocura.";
  }
